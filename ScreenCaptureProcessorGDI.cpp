...
bool ScreenCaptureProcessorGDI::init() {
		...
		/*Интерфейс IDXGIDevice разработан для использования объектами DXGI, которым необходим доступ
		к другим объектам DXGI. Этот интерфейс полезен для приложений, которые не используют Direct3D для связи с DXGI.
		Функции создания устройства Direct3D возвращают объект устройства Direct3D. Этот объект устройства Direct3D 
		реализует интерфейс IUnknown. Вы можете запросить этот объект устройства Direct3D для соответствующего 
интерфейса IDXGIDevice устройства. Чтобы получить интерфейс IDXGIDevice устройства Direct3D, используйте следующий код:*/
