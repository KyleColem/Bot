...
/*
"Нажимает" клавишу в нужном окне
решение в целом не очень изящное, но работает,  и в целом делает что нужно
набор клавиш пока не полный, только заглавные англ. буквы, цифры, F1-F8 по идее работают, остальные не проверял
*/
/*конкретно в wow работает через раз, хз почему. HWND_BROADCAST работает всегда, но это не лучший вариант*/
PostKey::PostKey()
{
...
bool PostKey::PressKey(HWND win,  char* ch) { 
	//по строке найти позицию в массиве
	// сформировать WM_KEYDOWN_LPARAM И WM_KEYUP_LPARAM
	//отправить сообщения
	int i = FindCode(ch);
	if (i >= 0 && i < 71) {
		__int64 temp = arrayKey[i].scanCode;
		__int64 tmpWP = temp*65536;
		WPARAM WM_KEYM = (WPARAM)arrayKey[i].charCode;
		PostMessage(win, WM_KEYDOWN, WM_KEYM, tmpWP);
		RSleep();
		PostMessage(win, WM_KEYUP, WM_KEYM, (tmpWP+1) | (3221225472));
		return true;
	}
	else return false;
...
bool PostKey::PressKey(DWORD win, char* ch) {
	//по строке найти позицию в массиве
	// сформировать WM_KEYDOWN_LPARAM И WM_KEYUP_LPARAM
	//отправить сообщения
	int i = FindCode(ch);
	if (i >= 0 && i < 71) {
		...
		__int64 tmpWP = temp * 65536;
		WPARAM WM_KEYM = (WPARAM)arrayKey[i].charCode;
		PostThreadMessage(win, WM_KEYDOWN, WM_KEYM, tmpWP);
		RSleep();
		PostThreadMessage(win, WM_KEYUP, WM_KEYM, (tmpWP + 1) | (3221225472));
		return true;
	}
	else return false;
